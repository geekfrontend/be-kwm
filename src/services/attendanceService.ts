...
export const getAttendanceByUserId = async (userId: string) => {
  ...
  if (records.length === 0) {
    const err = new Error("Data presensi user tidak ditemukan") as Error & {
      status?: number;
    };
    err.status = 404;
    throw err;
...
export const markMealAllowancePaid = async (attendanceId: string) => {
  const now = new Date();

  const result = await prisma.mealAllowance.updateMany({
    where: {
      attendanceId,
      isEligible: true,
      isPaid: false,
